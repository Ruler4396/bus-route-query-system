...
function frameMetrics(minGap) {
  ...
  return {
    ...
    hasSidebar: !!q('.account-sidebar'),
    hasAvatarRow: !!q('.account-avatar-row'),
    swiperHidden: !swiper || window.getComputedStyle(swiper).display === 'none'
  };
}

test.describe('User Center Layout', () => {
  for (const viewport of [
    { width: 796, height: 780, minGap: 12 },
    { width: 520, height: 820, minGap: 8 }
  ]) {
    test(`user center should stay within viewport at ${viewport.width}px`, async ({ page }) => {
      await page.setViewportSize({ width: viewport.width, height: viewport.height });
      await loginToShell(page);
      const frame = await openFramePage(page, './pages/yonghu/center.html', /yonghu\/center\.html/);
      ...
      expect(metrics.hasSidebar).toBeTruthy();
      expect(metrics.hasAvatarRow).toBeTruthy();
