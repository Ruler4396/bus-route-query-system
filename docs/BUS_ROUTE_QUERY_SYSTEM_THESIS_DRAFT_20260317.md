...
结合本地项目文档、GitHub `main` 分支提交树、后端控制器接口与 `ui-automation/tests` 中的自动化脚本，可将第六章功能测试用例整理为表 6-2 至表 6-8。各表按“用户认证、路线推荐、地图实时能力、内容互动、后台管理、无障碍交互、界面回归守护”七类模块展开，既便于论文排版，也能与现有代码结构保持一致。

表 6-2 用户认证与个人中心测试用例

| 用例编号 | 测试内容 | 前置条件 | 测试步骤 | 预期结果 |
|---|---|---|---|---|
| TC-6-2-01 | 登录页响应式布局验证 | 浏览器可访问前台登录页 | 分别以 1280×900、820×860、390×844 打开登录页，检查登录卡片、访客入口、无障碍入口与字段标签显示情况 | 页面无水平溢出，登录表单完整可见，按钮宽度正常，访客入口与无障碍说明区域可正常显示 |
| TC-6-2-02 | 普通用户登录流程验证 | 已存在测试账号 `demo_user/demo123` | 输入账号密码并提交登录 | 成功进入前台壳层页，顶部导航与业务 `iframe` 加载正常，可继续访问个人中心等登录态页面 |
| TC-6-2-03 | 游客访问个人中心拦截验证 | 以游客身份进入首页 | 点击“个人中心”，先关闭登录提示，再次点击并选择去登录 | 首次弹出温和登录提示且不强制跳转；确认后跳转登录页，并保留原目标页 `iframeUrl` |
| TC-6-2-04 | 个人中心页面布局与信息展示验证 | 用户已登录 | 打开 `yonghu/center.html`，分别在平板与手机宽度下检查页头、侧栏、头像区与主内容区 | 页面无越界与遮挡，页头、侧栏、头像行均可见，移动端仍可完成信息查看与基础操作 |

表 6-3 路线查询与画像推荐测试用例

| 用例编号 | 测试内容 | 前置条件 | 测试步骤 | 预期结果 |
|---|---|---|---|---|
| TC-6-3-01 | 路线规划参数非空校验 | 已打开无障碍路线规划页 | 调用 `/route/plan` 或前端“生成方案”按钮时不填写起点或终点 | 系统返回“起点和终点站不能为空”提示，不生成错误推荐结果 |
| TC-6-3-02 | 轮椅 / 行动不便画像推荐验证 | 已进入路线规划页，起终点可命中试点站点 | 输入起终点关键词后选择 `WHEELCHAIR` 与 `AUTO`，点击生成方案 | 摘要区显示“轮椅 / 行动不便”，列表返回可推荐路线、已过滤路线、置信度、数据源和状态等信息 |
| TC-6-3-03 | 低视力画像差异化推荐验证 | 与上一用例相同 | 保持起终点不变，切换 `LOW_VISION` 后重新生成方案 | 推荐摘要显示“低视力”，首选路线与轮椅画像存在差异，说明画像权重已参与排序 |
| TC-6-3-04 | 分段建模结果展示验证 | 已生成至少 1 条推荐路线 | 查看首条方案的分段信息 | 结果区完整展示出发步行段、上车站可达性、公交乘车段、换乘设施评估、下车站可达性、到达步行段六类分段说明 |

表 6-4 地图展示与实时能力测试用例

| 用例编号 | 测试内容 | 前置条件 | 测试步骤 | 预期结果 |
|---|---|---|---|---|
| TC-6-4-01 | 路线地图主数据加载验证 | 数据库中存在有效路线数据 | 调用 `/map/route/{id}` 获取指定线路地图数据 | 返回路线编号、名称、起终点坐标、站点坐标、路线轨迹与途经站点等字段，前端可据此绘图 |
| TC-6-4-02 | 路线站点列表与页面布局验证 | 已进入“实时线路地图”页 | 选择 `31路`，等待地图与站点面板加载 | 站点列表数量不少于 20 个，地图容器与列表区无明显空白缝隙，页面主体布局稳定 |
| TC-6-4-03 | 实时车辆推送启动验证 | 已完成路线选择 | 点击“车辆运行/实时推送”按钮并等待数秒 | 按钮文本切换为“停止”类状态，车辆标记出现，状态栏显示“实时推送”或“模拟车辆/推送通道”提示 |
| TC-6-4-04 | ETA 可解释字段返回验证 | 已知路线 ID 可用 | 调用 `/map/eta/{id}`，传入当前站点索引 | 返回剩余站数、剩余距离、路况等级、均速、行驶时间、停站时间、预计到达时刻和计算时间等字段 |

表 6-5 留言、公告、资源与收藏测试用例

| 用例编号 | 测试内容 | 前置条件 | 测试步骤 | 预期结果 |
|---|---|---|---|---|
| TC-6-5-01 | 匿名留言默认用户名验证 | 未登录或未填写用户名 | 以前台接口 `/messages/add` 提交留言且省略 `username` | 后端自动补全为“匿名访客”，留言记录可成功保存 |
| TC-6-5-02 | 留言提交与处理看板联动验证 | 已进入留言页 | 填写反馈类型、路线名称、站点名称和内容后提交，再进入“反馈处理看板”修改状态为 `IN_REVIEW` | 留言提交成功，看板能看到新记录，状态、审核人和备注可正常保存并刷新 |
| TC-6-5-03 | 公告详情浏览与收藏登录提示验证 | 以游客身份进入系统 | 打开公告详情页并点击“点我收藏” | 游客可正常浏览公告内容，但在收藏动作触发时弹出登录提示，不直接报错或空白 |
| TC-6-5-04 | 资源详情与收藏页展示验证 | 登录用户可访问收藏页 | 打开资源详情页检查正文区，再进入收藏列表页 | 资源详情页无嵌套白框和多余装饰媒体；收藏页工具栏、列表或空态区域显示正常，页面无横向溢出 |

表 6-6 后台管理与数据导出测试用例

| 用例编号 | 测试内容 | 前置条件 | 测试步骤 | 预期结果 |
|---|---|---|---|---|
| TC-6-6-01 | 后台管理员登录验证 | 已存在管理员账号 | 打开后台登录页，输入管理员账号密码并登录 | 成功进入后台工作台，侧栏品牌区、一级导航与子导航正常渲染 |
| TC-6-6-02 | 后台主题与导航一致性验证 | 管理员已登录后台 | 展开侧栏菜单，检查选中项、徽标、子菜单与网格布局样式 | 后台主题与前台视觉语言保持统一，侧栏宽度、边框、分区标题和子菜单布局符合设计要求 |
| TC-6-6-03 | 公交路线数据导出验证 | 后台路线数据存在 | 调用 `/gongjiaoluxian/export` 或在后台点击“导出Excel” | 成功下载 `gongjiaoluxian-export.xlsx`，文件可打开，包含路线编号、路线名称、起终点、途经站点、无障碍字段与创建时间等列 |

表 6-7 无障碍交互与辅助功能测试用例

| 用例编号 | 测试内容 | 前置条件 | 测试步骤 | 预期结果 |
|---|---|---|---|---|
| TC-6-7-01 | 键盘快捷键导航验证 | 已进入前台壳层页 | 依次按下 `Alt+2`、`Alt+M`、`Alt+6`、`Alt+7`、`Alt+A`、`Alt+L` | 能依次切换到路线规划、地图、留言、资源链接、设置页，并将焦点落到主内容区域 |
| TC-6-7-02 | 键盘 Enter 打开路线详情验证 | 已进入路线列表页 | 将焦点定位到首条路线卡片并按 Enter | 当前 `iframe` 跳转到路线详情页，证明卡片支持键盘无障碍激活 |
| TC-6-7-03 | 字幕中心与视觉提示开关验证 | 已进入前台首页 | 通过 `Alt+C`、`Alt+/` 或辅助按钮打开字幕中心，再关闭并重新开启视觉提示 | 页面出现可见文本替代与快捷键帮助；视觉提示开关状态可持久化保存并可再次恢复显示 |
| TC-6-7-04 | 无障碍预设与语音兜底验证 | 已进入无障碍设置页 | 分别点击“低视力预设”“听障预设”；在原生语音不可用条件下点击“测试语音” | 低视力预设能开启高对比和大字号，听障预设能开启字幕中心、减少动效和键盘导航；原生语音不可用时可请求 `/accessibility/tts/audio` 后端音频兜底 |

表 6-8 界面布局与回归守护测试用例

| 用例编号 | 测试内容 | 前置条件 | 测试步骤 | 预期结果 |
|---|---|---|---|---|
| TC-6-8-01 | 壳层加载健康度验证 | 系统首页可访问 | 打开首页并检查顶部、主内容区与业务 `iframe` | 页面不应出现壳层空白、`iframe` 高度塌陷、头部遮挡主内容等问题 |
| TC-6-8-02 | 地图页空白滚动与冻结加载校验 | 已进入地图页或主导航可切换页面 | 检查地图页底部空白滚动距离，并多次切换主导航页面 | 地图页无过长空白滚动区，公告页、资源页和路线页切换时不会遗留冻结加载层 |
| TC-6-8-03 | 移动端语音委托与解锁验证 | 模拟移动端浏览器环境 | 打开前台首页，触发 `AccessibilityUtils.speak()`，再进行一次用户点击 | 语音请求先由宿主壳层接管，首次交互后语音能力被解锁，移动端场景下仍可完成播报 |
