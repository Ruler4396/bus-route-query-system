...
- 前台壳层入口版本提升到 `shell-page.js?v=20260316-650`。

## 2026-03-16 · CHG-20260316-649 · 登录页删除演示说明卡片
- 按演示现场反馈，前台登录页已移除表单上方的 `演示账号` 与 `公共功能无需登录` 两块说明卡，避免登录页视觉噪声过多、抢占输入区注意力。
- 本轮仅删除登录表单内部的两块辅助文案，不影响账号密码输入、游客浏览入口、管理员后台入口与注册入口。

## 2026-03-16 · CHG-20260316-648 · 用户注册流程精简为账号+密码
- 前台 `front/pages/yonghu/register.html` 已移除 `姓名 / 年龄 / 手机` 三个注册项，用户注册时仅需填写账号、密码和确认密码，减少演示与首次使用阻力。
- 前台提交逻辑同步收口：不再校验姓名、年龄、手机号格式；提交时会自动用账号回填 `xingming`，其余可选资料留空，避免旧表结构影响简化后的注册流程。
- 后端 `YonghuController#/register` 增加兜底：仅强制要求账号与密码；若请求未提供姓名，则自动回填为账号，并补齐 `addtime`，保证旧页面 / 直接接口调用都能兼容新注册方式。
- 验证时已补齐 MySQL 演示用户 `demo_user / demo123`，当前后台“用户管理”至少可看到 1 条测试用户记录。

## 2026-03-16 · CHG-20260316-647 · 合成换乘标记区分 + 非乘坐区间灰显
...
- 前台资源版本提升到 `shell-page.js?v=20260316-647`、`map.html?v=20260316-647`。
- 已用脚本校验当前 4 条 polyline 的图层顺序：灰底线 `zIndex=20`，彩色乘坐段 `zIndex=80`，确保灰线处于下层。
